    ...
    <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden flex flex-col h-full">
      ...
      <div className="grid grid-cols-1 gap-3 mt-auto">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <a
            href={`https://wa.me/?text=${encodeURIComponent(`Check out this job at AH Career: ${title}\n${typeof window !== 'undefined' ? window.location.origin : ''}/jobs/${slug || _id}`)}`}
            target="_blank"
            rel="noreferrer"
            className="bg-green-50 text-green-700 font-bold py-2.5 px-3 rounded-xl hover:bg-green-100 transition-all text-sm flex items-center justify-center gap-1 active:scale-95"
          >
          ...
          <Link 
            to={`/jobs/${slug || _id}`}
            className="bg-blue-50 text-blue-700 font-bold py-2.5 px-3 rounded-xl hover:bg-blue-100 transition-all text-sm flex items-center justify-center active:scale-95"
          >
            Details →
          </Link>
        </div>
